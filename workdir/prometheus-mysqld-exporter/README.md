# Устранение уязвимостей без изменения версии приложения: prometheus-mysqld-exporter
...
**Цель:** устранить все 3 находок из входного списка для `prometheus-mysqld-exporter-0.17.2-1.sl9^1`.

**Неизменяемая версия приложения:** `0.17.2`. Исходный RPM: `prometheus-mysqld-exporter-0.17.2-1.sl9^1`. Сохранить RPM `Version: 0.17.2` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить базу mysqld_exporter 0.17.2; обновить edwards25519 и сетевые библиотеки в исходниках экспортера. Проверить совместимость транзитивного MySQL-драйвера с новой edwards25519.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-prometheus-mysqld-exporter-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---:|---|---|---|---|---|
| 1 | CVE-2026-26958 | `filippo.io/edwards25519` | `v1.1.0` | 1.1.1 | [advisory](https://pkg.go.dev/vuln/GO-2026-4503) |
| 57 | CVE-2026-39821 | `golang.org/x/net` | `v0.33.0` | 0.55.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5026) |
| 61 | CVE-2026-33814 | `golang.org/x/net` | `v0.33.0` | 0.53.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-4918) |
...
targets = {'filippo.io/edwards25519': '1.1.1', 'golang.org/x/net': '0.58.0', 'golang.org/x/crypto': '0.56.0', 'golang.org/x/text': '0.41.0'}
def version_key(v):
...
- [ ] Проверить MySQL и MariaDB используемых версий, реальные режимы аутентификации, TLS, reconnect, timeout и сбор ключевых метрик /metrics. Сравнить имена, labels и значения collectors до/после на одном стенде. Неверные credentials должны давать управляемую ошибку без паники и утечки секрета.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `0.17.2`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://github.com/prometheus/mysqld_exporter).
- [Официальная база Go Vulnerability Database](https://vuln.go.dev/); точные карточки приведены у каждой CVE.
- [go.mod filippo.io/edwards25519 v1.1.1](https://proxy.golang.org/filippo.io/edwards25519/@v/v1.1.1.mod).
