# Устранение уязвимостей без изменения версии приложения: prometheus-openstack-exporter
...
**Цель:** устранить все 5 находок из входного списка для `prometheus-openstack-exporter-1.7.0-1.sl9^1`.

**Неизменяемая версия приложения:** `1.7.0`. Исходный RPM: `prometheus-openstack-exporter-1.7.0-1.sl9^1`. Сохранить RPM `Version: 1.7.0` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить базу openstack-exporter 1.7.0; обновить x/net и x/text вместе с согласованными транзитивными версиями. Особое внимание уделить клиентам OpenStack и их HTTP transports/proxy.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-prometheus-openstack-exporter-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---|---|---|
| `golang.org/x/net` | `v0.10.0` | v0.58.0 |
| `golang.org/x/text` | `v0.9.0` | v0.41.0 |
...
|---:|---|---|---|---|---|
| 58 | CVE-2026-39821 | `golang.org/x/net` | `v0.10.0` | 0.55.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5026) |
| 70 | CVE-2025-22870 | `golang.org/x/net` | `v0.10.0` | 0.36.0 | [advisory](https://pkg.go.dev/vuln/GO-2025-3503) |
| 74 | CVE-2023-45288 | `golang.org/x/net` | `v0.10.0` | 0.23.0 | [advisory](https://pkg.go.dev/vuln/GO-2024-2687) |
| 78 | CVE-2023-39325 | `golang.org/x/net` | `v0.10.0` | 0.17.0 | [advisory](https://pkg.go.dev/vuln/GO-2023-2102) |
| 84 | CVE-2026-56852 | `golang.org/x/text` | `v0.9.0` | 0.39.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5970) |
...
targets = {'golang.org/x/net': '0.58.0', 'golang.org/x/text': '0.41.0', 'golang.org/x/crypto': '0.56.0'}
def version_key(v):
...
- [ ] На тестовом OpenStack проверить Keystone auth/обновление токена, используемые Nova/Neutron/Cinder/Glance collectors, pagination, timeout, reconnect, TLS и корпоративный HTTP proxy/NO_PROXY. Проверить ожидаемый запрет запросов к исключённым proxy адресам для CVE-2025-22870. Сравнить series/labels и ошибки /metrics до/после на одинаковой конфигурации clouds.yaml.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `1.7.0`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://github.com/openstack-exporter/openstack-exporter).
