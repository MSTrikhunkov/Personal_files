# Устранение уязвимостей без изменения версии приложения: prometheus-memcached-exporter
...
**Цель:** устранить все 3 находок из входного списка для `prometheus-memcached-exporter-0.15.5-1.sl9^1`.

**Неизменяемая версия приложения:** `0.15.5`. Исходный RPM: `prometheus-memcached-exporter-0.15.5-1.sl9^1`. Сохранить RPM `Version: 0.15.5` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить базу memcached_exporter 0.15.5; обновить x/net и x/text, согласовав транзитивные версии. Пересобрать сам exporter, а не только системный пакет golang.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-prometheus-memcached-exporter-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---|---|---|
| `golang.org/x/net` | `v0.47.0` | v0.58.0 |
| `golang.org/x/text` | `v0.31.0` | v0.41.0 |
...
|---:|---|---|---|---|---|
| 55 | CVE-2026-39821 | `golang.org/x/net` | `v0.47.0` | 0.55.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5026) |
| 59 | CVE-2026-33814 | `golang.org/x/net` | `v0.47.0` | 0.53.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-4918) |
| 83 | CVE-2026-56852 | `golang.org/x/text` | `v0.31.0` | 0.39.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5970) |
...
- [ ] Проверить /metrics на используемой версии Memcached, подключение, reconnect, timeout, недоступный сервер и включённые параметры TLS/auth. Сравнить имена и labels метрик до/после, проверить HTTP/HTTPS endpoint экспортера с действующей конфигурацией.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `0.15.5`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://github.com/prometheus/memcached_exporter).
