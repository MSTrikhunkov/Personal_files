# Устранение уязвимостей без изменения версии приложения: prometheus_libvirt_exporter
...
**Цель:** устранить все 17 находок из входного списка для `prometheus_libvirt_exporter-2.2.0-1.sl9^1`.

**Неизменяемая версия приложения:** `2.2.0`. Исходный RPM: `prometheus_libvirt_exporter-2.2.0-1.sl9^1`. Сохранить RPM `Version: 2.2.0` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить базу экспортера 2.2.0 и обновить x/crypto и x/text вместе с требуемыми x/net/x/sys/x/term. Выявить CGO-зависимости и BuildRequires libvirt из SRPM: один Go build без libvirt-devel и штатных тегов сборки не является проверкой пакета.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-prometheus-libvirt-exporter-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---|---|---|
| `golang.org/x/crypto` | `v0.39.0` | v0.56.0 |
...
|---:|---|---|---|---|---|
| 18 | CVE-2026-56855 | `golang.org/x/crypto` | `v0.39.0` | 0.56.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-6355) |
| 20 | CVE-2026-56854 | `golang.org/x/crypto` | `v0.39.0` | 0.55.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-6303) |
| 21 | CVE-2026-46598 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5033) |
| 23 | CVE-2026-46597 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5013) |
| 25 | CVE-2026-46595 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5023) |
| 28 | CVE-2026-39835 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5015) |
| 30 | CVE-2026-39834 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5020) |
| 32 | CVE-2026-39833 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5005) |
| 34 | CVE-2026-39832 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5006) |
| 36 | CVE-2026-39831 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5019) |
| 38 | CVE-2026-39830 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5017) |
| 40 | CVE-2026-39829 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5018) |
| 43 | CVE-2026-39827 | `golang.org/x/crypto` | `v0.39.0` | 0.52.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5016) |
| 45 | CVE-2025-58181 | `golang.org/x/crypto` | `v0.39.0` | 0.45.0 | [advisory](https://pkg.go.dev/vuln/GO-2025-4134) |
| 47 | CVE-2025-47914 | `golang.org/x/crypto` | `v0.39.0` | 0.45.0 | [advisory](https://pkg.go.dev/vuln/GO-2025-4135) |
| 49 | CVE-2025-47913 | `golang.org/x/crypto` | `v0.39.0` | 0.43.0 | [advisory](https://pkg.go.dev/vuln/GO-2025-4116) |
| 82 | CVE-2026-56852 | `golang.org/x/text` | `v0.26.0` | 0.39.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5970) |
...
- [ ] Сохранить базу экспортера 2.2.0 и обновить x/crypto и x/text вместе с требуемыми x/net/x/sys/x/term. Выявить CGO-зависимости и BuildRequires libvirt из SRPM: один Go build без libvirt-devel и штатных тегов сборки не является проверкой пакета.
- [ ] В фактически найденном spec сохранить `Version: 2.2.0` и Source базового приложения, увеличить только `Release` по правилам дистрибутива. Добавить необходимые Patch и обновлённый vendor Source-архив, сохранив патчи дистрибутива. Зафиксировать Go BuildRequires/образ сборщика и флаги сборки; проверить `rpm -qp --qf '%{VERSION}-%{RELEASE}\n'` для готового RPM.
...
targets = {'golang.org/x/crypto': '0.56.0', 'golang.org/x/text': '0.41.0', 'golang.org/x/net': '0.58.0'}
def version_key(v):
...
- [ ] Проверить сборку и запуск с libvirt используемой версии, подключение к локальному сокету и удалённому libvirt/TLS/SSH при наличии такой настройки, reconnect после перезапуска libvirtd. Сравнить метрики работающей, выключенной и удалённой VM; убедиться в отсутствии зависаний и потери collectors. Прогнать upstream regression x/crypto/ssh и x/text для перечисленных CVE.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `2.2.0`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://pkg.go.dev/golang.org/x/crypto).
...
- [go.mod golang.org/x/text v0.39.0](https://proxy.golang.org/golang.org/x/text/@v/v0.39.0.mod).
