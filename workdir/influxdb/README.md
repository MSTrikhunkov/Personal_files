# Устранение уязвимостей без изменения версии приложения: influxdb
...
**Цель:** устранить все 3 находок из входного списка для `influxdb-1.8.10-1.sl9^1`.

**Неизменяемая версия приложения:** `1.8.10`. Исходный RPM: `influxdb-1.8.10-1.sl9^1`. Сохранить RPM `Version: 1.8.10` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить InfluxDB 1.8.10 и подготовить пересборку этой базы с обновлённым gRPC. Пакет взят из STAK-kolla-toolbox: обновление toolbox без пересборки influxdb не меняет статически включённые Go-библиотеки. Переход gRPC 1.26.0 → 1.83.2 значителен: проверить grpc-gateway и protobuf/generated stubs. При удалённых API обновить их потребителей или генератор и пересоздать stubs из исходных .proto; не возвращать уязвимый gRPC. Переход на InfluxDB 2/3 не является частью этого плана.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-kolla-toolbox-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---|---|---|
| `google.golang.org/grpc` | `v1.26.0` | v1.83.2 |
...
|---:|---|---|---|---|---|
| 90 | CVE-2026-84445 | `google.golang.org/grpc` | `v1.26.0` | 1.82.2 / 1.83.2 / 1.85.0-dev.0.20260825072537-93e31b48545e | [advisory](https://pkg.go.dev/vuln/GO-2026-6443) |
| 95 | CVE-2026-84304 | `google.golang.org/grpc` | `v1.26.0` | 1.83.1 | [advisory](https://pkg.go.dev/vuln/GO-2026-6348) |
| 98 | CVE-2026-84303 | `google.golang.org/grpc` | `v1.26.0` | 1.83.1 | [advisory](https://pkg.go.dev/vuln/GO-2026-6441) |
...
- [ ] Сохранить InfluxDB 1.8.10 и подготовить пересборку этой базы с обновлённым gRPC. Пакет взят из STAK-kolla-toolbox: обновление toolbox без пересборки influxdb не меняет статически включённые Go-библиотеки. Переход gRPC 1.26.0 → 1.83.2 значителен: проверить grpc-gateway и protobuf/generated stubs. При удалённых API обновить их потребителей или генератор и пересоздать stubs из исходных .proto; не возвращать уязвимый gRPC. Переход на InfluxDB 2/3 не является частью этого плана.
- [ ] В фактически найденном spec сохранить `Version: 1.8.10` и Source базового приложения, увеличить только `Release` по правилам дистрибутива. Добавить необходимые Patch и обновлённый vendor Source-архив, сохранив патчи дистрибутива. Зафиксировать Go BuildRequires/образ сборщика и флаги сборки; проверить `rpm -qp --qf '%{VERSION}-%{RELEASE}\n'` для готового RPM.
...
targets = {'google.golang.org/grpc': '1.83.2', 'golang.org/x/crypto': '0.56.0', 'golang.org/x/net': '0.58.0', 'golang.org/x/text': '0.41.0', 'go.opentelemetry.io/otel': '1.44.0', 'go.opentelemetry.io/otel/metric': '1.44.0', 'go.opentelemetry.io/otel/trace': '1.44.0', 'go.opentelemetry.io/otel/sdk': '1.44.0', 'go.opentelemetry.io/otel/sdk/metric': '1.44.0'}
def version_key(v):
...
- [ ] Проверить influxd и все дополнительные Go-бинарники RPM, HTTP /ping, запись line protocol, InfluxQL, auth/TLS, retention policies, continuous queries и штатные backup/restore. Проверить RPC между внутренними компонентами и вызовы из kolla-toolbox на реальном тестовом сценарии. Сделать backup данных и конфигурации; прогнать восстановление в изолированном окружении. При несовместимости новой библиотеки выпуск блокируется до адаптации кода и успешной регрессии.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `1.8.10`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://github.com/influxdata/influxdb/tree/v1.8.10).
