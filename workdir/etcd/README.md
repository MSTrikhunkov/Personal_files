# Устранение уязвимостей без изменения версии приложения: etcd
...
**Цель:** устранить все 4 находок из входного списка для `etcd-3.5.33-1.sl9^1^1`.

**Неизменяемая версия приложения:** `3.5.33`. Исходный RPM: `etcd-3.5.33-1.sl9^1^1`. Сохранить RPM `Version: 3.5.33` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить базу etcd 3.5.33 и обновить vendored-зависимости. Найти все go.mod/go.work исходного SRPM: etcd многомодульный, обновление одного корневого go.mod не доказывает обновление server, client, etcdctl и etcdutl. Сохранить локальные replace между модулями etcd; одинаковые внешние зависимости согласовать во всех сборочных модулях.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-etcd-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---:|---|---|---|---|---|
| 5 | GHSA-w67g-5rqw-f597 | `github.com/gorilla/websocket` | `v1.4.2` | 1.5.3 | [advisory](https://pkg.go.dev/vuln/GO-2026-6278) |
| 15 | CVE-2026-41178 | `go.opentelemetry.io/otel` | `v1.43.0` | 1.42.0 / 1.44.0 | [advisory](https://pkg.go.dev/vuln/GO-2026-5158) |
| 87 | CVE-2026-84445 | `google.golang.org/grpc` | `v1.82.1` | 1.82.2 / 1.83.2 / 1.85.0-dev.0.20260825072537-93e31b48545e | [advisory](https://pkg.go.dev/vuln/GO-2026-6443) |
| 92 | CVE-2026-84304 | `google.golang.org/grpc` | `v1.82.1` | 1.83.1 | [advisory](https://pkg.go.dev/vuln/GO-2026-6348) |
...
- [ ] Сохранить базу etcd 3.5.33 и обновить vendored-зависимости. Найти все go.mod/go.work исходного SRPM: etcd многомодульный, обновление одного корневого go.mod не доказывает обновление server, client, etcdctl и etcdutl. Сохранить локальные replace между модулями etcd; одинаковые внешние зависимости согласовать во всех сборочных модулях.
- [ ] В фактически найденном spec сохранить `Version: 3.5.33` и Source базового приложения, увеличить только `Release` по правилам дистрибутива. Добавить необходимые Patch и обновлённый vendor Source-архив, сохранив патчи дистрибутива. Зафиксировать Go BuildRequires/образ сборщика и флаги сборки; проверить `rpm -qp --qf '%{VERSION}-%{RELEASE}\n'` для готового RPM.
...
targets = {'github.com/gorilla/websocket': '1.5.3', 'go.opentelemetry.io/otel': '1.44.0', 'google.golang.org/grpc': '1.83.2', 'golang.org/x/crypto': '0.56.0', 'golang.org/x/net': '0.58.0', 'golang.org/x/text': '0.41.0', 'go.opentelemetry.io/otel/metric': '1.44.0', 'go.opentelemetry.io/otel/trace': '1.44.0', 'go.opentelemetry.io/otel/sdk': '1.44.0', 'go.opentelemetry.io/otel/sdk/metric': '1.44.0'}
def version_key(v):
...
- [ ] На отдельном трёхузловом кластере проверить etcdctl endpoint health/status, put/get/watch, leases, transactions, auth/RBAC, TLS/mTLS, snapshot save/restore и смену лидера. Прогнать штатные integration/e2e etcd для gRPC client/server, keepalive и больших сообщений. Проверить gateway/WebSocket при его использовании и OTEL exporters. Снять snapshot и проверить восстановление; обновлять по одному узлу без потери quorum. Проверить версии библиотек в каждом из etcd, etcdctl, etcdutl, если они входят в RPM.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `3.5.33`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://etcd.io/docs/v3.5/upgrades/).
- [Официальная база Go Vulnerability Database](https://vuln.go.dev/); точные карточки приведены у каждой CVE.
- [go.mod github.com/gorilla/websocket v1.5.3](https://proxy.golang.org/github.com/gorilla/websocket/@v/v1.5.3.mod).
- [go.mod go.opentelemetry.io/otel v1.44.0](https://proxy.golang.org/go.opentelemetry.io/otel/@v/v1.44.0.mod).
