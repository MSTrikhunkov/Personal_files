# Устранение уязвимостей без изменения версии приложения: blackbox_exporter
...
**Цель:** устранить все 2 находок из входного списка для `blackbox_exporter-0.26.0-2.sl9^2^1`.

**Неизменяемая версия приложения:** `0.26.0`. Исходный RPM: `blackbox_exporter-0.26.0-2.sl9^2^1`. Сохранить RPM `Version: 0.26.0` и исходную базу приложения; для пересборки разрешено увеличить только `Release`. Нельзя подменять исходники новым релизом или повышать версию собственного main-модуля, в том числе для обхода несовместимости зависимостей.

**Архитектура:** Сохранить исходную базу 0.26.0 и обновить gRPC и его зависимости в исходном пакете blackbox_exporter. Сохранить патчи дистрибутива и конфигурацию модулей проб.

**Стек:** Go, Go modules/vendor, RPM для sl9, образ STAK. Дата проверки источников: 17.09.2026.

**Спецификация:** [исходный список](../findings.tsv); идентификатор образа `STAK-prometheus-blackbox-exporter-pvs___1.0-20260914.0422`. Группировка выполнена строго по последнему столбцу, включая суффиксы `^`.
...
|---:|---|---|---|---|---|
| 89 | CVE-2026-84445 | `google.golang.org/grpc` | `v1.70.0` | 1.82.2 / 1.83.2 / 1.85.0-dev.0.20260825072537-93e31b48545e | [advisory](https://pkg.go.dev/vuln/GO-2026-6443) |
| 94 | CVE-2026-84304 | `google.golang.org/grpc` | `v1.70.0` | 1.83.1 | [advisory](https://pkg.go.dev/vuln/GO-2026-6348) |
...
- [ ] Проверить /metrics и /probe для HTTP/HTTPS, TCP, DNS, ICMP и gRPC. Для gRPC проверить health service с TLS/mTLS, неверным сертификатом, timeout и недоступным сервером; убедиться, что probe_success и labels совпадают с ожидаемыми. Проверить права/capabilities ICMP в RPM/образе. Выпускать canary с существующей blackbox.yml.
...
- [ ] Исходники, vendor, фактические бинарники и SBOM согласованы; приложение и RPM `Version` остаются `0.26.0`, изменён `Release`, сохранены новые checksums/digest RPM и образа.
...
- [Upstream / документация продукта](https://github.com/prometheus/blackbox_exporter).
